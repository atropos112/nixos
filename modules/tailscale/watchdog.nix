{
  ...
  # ============================================================================
  # Watchdog: restart tailscaled if peer stuck on relay too long
  # ============================================================================
  # Works with tailscale-keepalive which handles pinging.
  # This just monitors relay duration and restarts if threshold exceeded.
  # ============================================================================
  watchdogScript = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      # HELP tailscale_watchdog_peer_state Current state per peer (0=direct, 1=relay)
...
in {
  options.atro.tailscale.watchdog = {
    enable = mkEnableOption "Tailscale watchdog (restart if stuck on relay)";
    ...
    maxRelaySeconds = mkOption {
      type = types.int;
      default = 600;
      description = "Seconds on relay before restarting tailscaled";
    };
  ...
  config = mkIf cfg.enable {
    systemd = {
      ...
      services.tailscale-watchdog = {
        description = "Tailscale watchdog (restart if stuck on relay)";
